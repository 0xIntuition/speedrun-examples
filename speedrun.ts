...
const apiUrl = (process.env.API_URL as string) ?? 'https://api.intuition.cafe'

// Authentication
// Gets SIWE message to sign
async function getMessage(apiKey: string, wallet: `0x${string}`): Promise<string | null> {
  try {
    const response = await fetch(`${apiUrl}/auth/message?wallet=${encodeURIComponent(wallet as string)}`, {
...
async function getSession(msg: string, signature: string, wallet: `0x${string}`): Promise<string | null> {
  const response = await fetch(
    `${apiUrl}/auth/session?message=${encodeURIComponent(msg)}&signature=${encodeURIComponent(
      signature
    )}&wallet=${encodeURIComponent(wallet as string)}`,
    {
      headers: {
...
async function attestToClaim(session: string, claim_id: string, direction: boolean): Promise<void> {
  const response = await fetch(`${apiUrl}/claim/attest`, {
    method: 'POST',
    headers: {
      Authorization: 'Bearer ' + session,
      'x-api-key': process.env.API_KEY as string,
...
// Querying Identities
// Attests to the specified Claim in the direction (true: for, false: against)
async function queryIdentitiesByDisplayName(session: string, display_name: string): Promise<any> {
  const response = await fetch(`${apiUrl}/query/identities`, {
    method: 'POST',
    headers: {
      Authorization: 'Bearer ' + session,
      'x-api-key': process.env.API_KEY as string,
...
async function queryClaimsByCreator(session: string, wallet: string): Promise<any> {
  const response = await fetch(`${apiUrl}/query/claims`, {
    method: 'POST',
    headers: {
      Authorization: 'Bearer ' + session,
      'x-api-key': process.env.API_KEY as string,
...
async function main() {
  ...
  let claim_id = await createClaim(
    session as string,
    subject_id,
    '50ba5a74941e6456184e9fb1dbfec124eb3bd6e96fab0bdcbeac5de2da191180',
    object_id
  )
  ...
  await queryIdentitiesByDisplayName(session as string, 'Intuition')

  // Query for Claims by Creator
  await queryClaimsByCreator(session as string, '0xCeBcD6631947E46FEEFccA6A4D96468695cb8540')
}
